(function(f) {
	...
	var t = function() {}, q = f.getOptions(), h = f.each, l = f.extend, B = f.format, u = f.pick, r = f.wrap, m = f.Chart, p = f.seriesTypes, v = p.pie, n = p.column, w = f.Tick, x = HighchartsAdapter.fireEvent, y = HighchartsAdapter.inArray,
			z = 1;
	h(["fill", "stroke"],
			function(b) {HighchartsAdapter.addAnimSetter(b, function(a) {a.elem.attr(b, A(f.Color(a.start), f.Color(a.end), a.pos))})});
	...
	m.prototype.applyDrilldown = function() {
		...
		if (b && b.length > 0) {
			a = b[b.length - 1].levelNumber, h(this.drilldownLevels,
					function(b) {
						b.levelNumber === a && h(b.levelSeries, function(b) {b.options && b.options._levelNumber === a && b.remove(!1)})
					});
		}
		this.redraw();
	...
	m.prototype.showDrillUpButton = function() {
		...
		this.drillUpButton ? this.drillUpButton.attr({text : a}).align() :
				(g = (d = c.theme) && d.states, this.drillUpButton =
						this.renderer.button(a, null, null, function() {b.drillUp()}, d, g && g.hover,
	...
	m.prototype.drillUp = function() {
		for (var b = this, a = b.drilldownLevels, c = a[a.length - 1].levelNumber, d = a.length, g = b.series, e, i, f, j, k = function(a) {
			var c;
			h(g, function(b) {b.options._ddSeriesId === a._ddSeriesId && (c = b)});
			c = c || b.addSeries(a, !1);
	...
	f.Point.prototype.doDrilldown = function(b, a) {
		...
		x(c, "drilldown", {point : this, seriesOptions : e, category : a, points : a !== void 0 && this.series.xAxis.ddPoints[a].slice(0)});
	...
	w.prototype.drillable = function() {
		...
		if (a && d && d.length) {
			...
			a.addClass("highcharts-drilldown-axis-label").css(c.chart.options.drilldown.activeAxisLabelStyle).on("click",
					function() {c.drilldownCategory(b)})
	...
	r(f.Series.prototype, "drawDataLabels", function(b) {
		...
		b.call(this);
		h(this.points, function(b) {b.drilldown && b.dataLabel && b.dataLabel.attr({"class" : "highcharts-drilldown-data-label"}).css(a)})
	});
	var s, q = function(b) {
		b.call(this);
		h(this.points, function(a) {
